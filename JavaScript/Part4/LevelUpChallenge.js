...
cities.forEach(city => {
    if (city.toLowerCase() === 'paris') {
        return; // this will skip the current iteration and move to the next one
    }
...
let arrTypes = ["chai", "green tea", "black tea", "jasmine tea", "herbal tea"];
let shortTeas = [];
for (let tea of arrTypes){
    if (tea.length > 10){
        break;
    }
    shortTeas.push(tea);
